...
if(($Quantite == "") || ($Quantite == 0) || ($Quantite<0) ) {
  alert_and_redirect("Veuillez entrer une quantité correcte", "ListeLivre.php");
}
...
if($erreur == "") {
  ...
  if($db_found){
    $sql1 = "SELECT quantite, categorie FROM Items where id_item=$id_item";
    $result1 = mysqli_query($db_handle, $sql1) or die(mysqli_error($db_handle));
    while($data = mysqli_fetch_assoc($result1)){
      if ($Quantite>$data['quantite']) {
        alert_and_redirect("Quantité insuffisante", "ListeLivre.php");
      }
      else {
        $sql = "INSERT INTO Panier(id_acheteur, id_item, quantite_voulu) VALUES ($id_ach, $id_item, $Quantite)" or die(mysqli_error($db_handle));
        $result = mysqli_query($db_handle, $sql);
        alert_and_redirect("Objet ajouté au panier!","acceuilAcheteur.php");
      }
