...
if($db_found){
  ...
  $sql_qte = "SELECT quantite_voulu, p.id_item FROM Panier p WHERE id_acheteur=$id_ach";
  $result_qte = mysqli_query($db_handle, $sql_qte) or die(mysqli_error($db_handle));

  while($row = $result_qte->fetch_assoc()) {
    $id_item = $row['id_item'];
    $Quant = $row['quantite_voulu'];
    $sql_upd= "UPDATE Items SET quantite = quantite - $Quant, nb_vendu = nb_vendu + $Quant where id_item = $id_item";
    $result_upd = mysqli_query($db_handle, $sql_upd) or die(mysqli_error($db_handle));

    if (!$result_upd) {
      alert_and_redirect("Il y a eu une erreur", "panier.php");
    }
/*
    if ($row['quantite_vou'] - $Quant == 0) { //FIXME
      $sql_delete = "DELETE FROM items WHERE id_item = '$id_item'";
      $result = mysqli_query($db_handle, $sql_delete);
    }
    */

    $sql_del = "DELETE FROM Panier WHERE id_acheteur = $id_ach AND id_item = '$id_item'";
    $result_del = mysqli_query($db_handle, $sql_del) or die(mysqli_error($db_handle));

    alert_and_redirect("Votre commande a été passée successivement !", "acceuil.html");
  }
